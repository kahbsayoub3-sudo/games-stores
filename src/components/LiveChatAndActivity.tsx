...
import { Users, Send, MessageSquare, ShieldCheck, Activity, Zap } from 'lucide-react';
...
export default function LiveChatAndActivity() {
  ...
  const handleSendChat = (e: FormEvent) => {
    ...
    const chosenGame = targetGame.trim() || 'All items';
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900 shadow-xl flex flex-col justify-between h-[450px]">
      <div>
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-neutral-800">
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold font-display transition-colors cursor-pointer ${
                currentTab === 'chat'
            ...
            {currentTab === 'chat' ? (
              chatMessages.map((msg) => (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="flex gap-2.5 text-xs rounded-lg p-2.5 bg-neutral-50/50 dark:bg-neutral-950/20 border border-gray-100 dark:border-neutral-800/20"
                >
                  {/* Small Profile Initial Avatar */}
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-neutral-200 dark:bg-neutral-800 font-display font-black text-neutral-600 dark:text-neutral-300">
                    {msg.user.substring(0, 1).toUpperCase()}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-neutral-800 dark:text-neutral-200 font-display text-[11px]">
                        @{msg.user}
                      </span>
                      <span className="text-[9px] text-cyan-500/90 font-mono bg-cyan-500/5 px-1.5 rounded">
                        {msg.game}
                      </span>
                    </div>
                    <p className="mt-1 text-neutral-500 dark:text-neutral-400 leading-relaxed text-[11px]">
                      {msg.text}
                    </p>
                  </div>
                </motion.div>
              ...
              activities.map((act) => (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="flex items-center gap-3 text-xs p-2 rounded-lg bg-emerald-500/[0.01] border border-emerald-500/5 dark:bg-emerald-500/[0.02]"
                >
                  <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-500 shrink-0">
                    <Zap className="h-4 w-4 animate-bounce" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-neutral-600 dark:text-neutral-300 leading-normal">
                      <strong className="text-neutral-950 dark:text-white font-bold">@{act.user}</strong> sideloaded{' '}
      ...
      <form onSubmit={handleSendChat} id="chat-entry-form" className="mt-2 border-t border-gray-100 dark:border-neutral-800 pt-3">
        <div className="flex gap-2">
          <input
            type="text"
            id="shout-input-text"
            ...
            maxLength={100}
            className="flex-1 rounded-lg border border-gray-200 bg-transparent px-3 py-1.5 text-xs text-neutral-900 outline-none focus:border-cyan-500 dark:border-neutral-800 dark:text-white"
          />
          <button
            type="submit"
